...
**seccomp(secure computing)** 는 프로세스가 호출할 수 있는 시스템콜을 제한하는 오래된 리눅스 기능이다. 초기 seccomp(2005년의 strict 모드)는 `read`·`write`·`exit`·`sigreturn` **딱 4개**만 허용해 쓸모가 좁았다. **seccomp-bpf**(2012년)는 여기에 (고전) BPF 필터를 붙여, **"이 프로세스는 이 시스템콜만 허용"** 같은 규칙을 유연하게 걸 수 있게 한 진화형이다. 컨테이너 런타임이 컨테이너의 공격 표면을 줄일 때 흔히 쓴다.
...
**LSM BPF(BPF LSM)** 는 이 LSM 훅에 **eBPF 프로그램**을 달 수 있게 한 기능(커널 5.7+)이다. eBPF 가 보안 결정을 내려 **0(허용) 또는 음수 에러(거부)** 를 반환한다. 다만 트레이드오프가 있다. BPF LSM 은 부팅 시 커널 파라미터 `lsm=...,bpf` 로 **미리 활성화**돼 있어야 쓸 수 있고 런타임에 켤 수 없다 — 이 강의 VM 커널은 그 목록에 `bpf` 가 없어, "강제(차단)" 실습은 [고급 단원](../../labs/08_고급/README.md)의 XDP 드롭으로 대체한다.
