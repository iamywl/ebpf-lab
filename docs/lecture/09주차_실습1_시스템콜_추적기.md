...
> 🧭 **이번 주 동선**  ·  📘 과목1 8주차 🟢 (실습①)  ·  📕 과목2 해당 없음
> - 🔬 **실습(VM `ssh ossca-ebpf`)**: `cd projects/syscall-tracer && sudo python3 verify.py`
> - 🧵 **OS 트랙 함께 보기**: [OS V2 시스템콜](os/V2_제한적직접실행과_시스템콜.md)
> - ↔️ **이동**: ⬅️ [8주차 BCC 입문](08주차_BCC_입문_맵과_perf이벤트.md) · 🏠 [강의 인덱스](README.md) · ➡️ [10주차 실습② 네트워크 추적기](10주차_실습2_네트워크_연결_추적기.md)

> 🔰 **1학년·입문자: 두 갈래 길로 읽자.**
> - **🟢 돌려보기 트랙(누구나)**: [§5 직접 실행해 보기](#5-직접-실행해-보기)의 명령을 복붙해 추적기를 실제로 돌리고, PASS/숫자만 읽어도 이번 주 성취는 충분하다. C 몰라도 된다.
...
- 더 깊은 이유 두 가지. ① **PID 재사용**: 짧게 살다 죽은 PID 는 곧 다른 프로세스에 재할당될 수 있다. 사용자 공간이 나중에 `/proc/<pid>/comm` 을 읽으면 **엉뚱한 새 프로세스의 이름**을 가져온다. 이벤트가 발생한 그 순간 커널 안에서 이름을 박제해 두면 이 혼동이 없다. ② **경계 전환 0회**: 이름을 커널에서 미리 받아두면, 사용자 공간이 매 PID 마다 `/proc` 를 여닫는 추가 시스템콜이 사라진다 — 집계가 "커널 안에서 끝난다"는 설계와 일관된다.
...
> **동시성·원자성 더 보기.** `(*val)++` 는 기계어로 "읽기 → +1 → 쓰기"(read-modify-write)의 세 단계다. 같은 (tgid, syscall) 키를 **여러 CPU 가 동시에** 올리면, 두 CPU 가 같은 값을 읽고 각자 +1 해 쓰는 바람에 한 번이 사라지는 **lost update** 가 이론상 가능하다. 두 가지 해법이 있다.
> - **원자 연산**: `lock_xadd(val, 1)`(BCC 가 제공하는 원자 덧셈)으로 바꾸면 경합을 없앤다. 약간의 동기화 비용이 든다.
> - **per-CPU 맵**: `BPF_HASH` 대신 `BPF_PERCPU_HASH` 를 쓰면 **CPU마다 별도 카운터**라 애초에 경합이 없다. 대신 사용자 공간이 읽을 때 **모든 CPU 값을 합산**해야 한다(읽기 쪽이 조금 복잡해짐).
>
> 이 실습이 평범한 해시맵을 쓰고도 통과하는 이유는, 동일 (tgid, syscall) 키를 같은 순간에 여러 CPU 가 때리는 경우가 드물고(단일 워크로드 프로세스는 대개 한 CPU 에서 순차 실행), 설령 어긋나도 그 오차가 **호출 횟수 N 에 비례하지 않는 소량**이라 §4 의 상한 검사(`+200`) 안에 들기 때문이다. 고정밀·고경합 환경이라면 위 두 대안 중 하나를 택한다.
...
> **왜 이 네 기둥이 서로를 보강하나 (속성으로 다시 보기).**
> - **기준값의 비조작성**: 정답이 측정 도구(eBPF)와 **다른 프로그램**(workload)의 소스에 상수로 박혀 있다. 추적기 코드를 아무리 고쳐도 기준값은 변하지 않으므로, 추적기가 자기 채점표를 바꿀 길이 없다.
> - **커널/사용자 독립 교차검증**: 두 측정이 **다른 공간·다른 코드·다른 메커니즘**(커널 트레이스포인트 카운트 vs 사용자 공간 루프 카운트)으로 같은 사건을 센다. 두 독립 경로가 동일 결론에 이르면, 둘이 똑같은 방식으로 동시에 틀렸을 확률은 사실상 0이다.
> - **스케일 불변성**: 오차가 N 에 **비례하지 않고 상수**임을 N 을 바꿔가며 확인한다. 비례 오차(=진짜 누락)와 고정 오차(=시작 비용)를 **수학적으로 분리**하는 장치다. 그리고 상한 `+200` 이 이 분리를 **자동으로 강제**한다.
> - **양방향 누락·과다 검사**: 하한은 "놓치지 않았나", 상한은 "부풀리지 않았나"를 본다. 한 방향만 보는 검증은 자명한 우회로(모두 0, 또는 무한 증가)를 허용하지만, 두 부등식이 동시에 성립해야 하므로 그런 우회가 막힌다.
>
> 정리하면, **무엇이 정답인지(비조작 기준값)**, **누가 측정했는지(독립 두 경로)**, **얼마나 키워도 견디는지(스케일 불변)**, **어느 방향으로도 안 틀리는지(양방향)** 를 한꺼번에 보장한다. "동작하는 것 같다"와 "동작함을 증명했다"의 차이가 여기에 있다.
...
> 아래는 이 추적기를 **실제 터미널에서 실행한 화면을 그대로 캡처**한 것이다. 코드를 못 읽어도 이 결과만 봐도 충분하다.

![tracer.py — PID(프로세스)별로 시스템콜을 분류해 실시간 집계한 실제 출력](images/shot_tracer_live.png)

![execsnoop.bt — 새로 실행되는 프로그램을 잡는 시스템콜 추적의 또 다른 예](images/shot_execsnoop.png)
...
> `ssh ossca-ebpf` 로 접속해 `~/ebpf-labs/projects/syscall-tracer` 에서 진행한다. 모두 `sudo` 필요.

**심화 1. 스케일 불변성을 손으로 확인 — 3,000 vs 100,000.**

- 목표: 호출 횟수를 33배 늘려도 "차이" 칼럼이 변하지 않음을 직접 본다(§4.3 의 핵심 근거).
- 명령:
  ```bash
  cd ~/ebpf-labs/projects/syscall-tracer
  sudo python3 verify.py 3000
  sudo python3 verify.py 100000
  ```
- 관찰: 두 실행의 출력에서 같은 시스템콜(예: `close`, `openat`, `getppid`)의 **"차이" 칼럼**을 나란히 비교한다.
- 질문: 호출량이 33배인데 차이가 그대로라면, 그 차이는 추적기 오류인가 아닌가? 만약 추적기가 0.1%씩 놓친다면 100,000회에서 차이는 대략 얼마가 되며 상한 `+200` 을 넘는가?

**심화 2. 실시간 추적 관찰 — 상위 8개.**

- 목표: 살아있는 시스템에서 어떤 프로세스가 어떤 시스템콜로 바쁜지 본다.
- 명령:
  ```bash
  sudo python3 tracer.py --duration 5 --top 8
  ```
- 관찰: PID별로 **총 호출 수 내림차순** 정렬과, 각 PID 안에서 시스템콜이 횟수 순으로 8개까지 나오는지 본다. `tracer.py` 자신(python3)이 `bpf` 시스템콜을 많이 부르는 것도 확인.
- 질문: 추적 도구 자신이 목록에 보이는 이유는? (자기 관측 = self-observation) 이를 빼고 싶다면 커널·사용자 어느 단계에서 거르는 게 가벼운가?

**심화 3. 코드 수정 — 특정 시스템콜만 세도록 BPF 필터 추가.**

- 목표: `syscall_count.c` 에 시스템콜 번호 필터를 더해 "원하는 시스템콜만" 집계한다(위 확장 과제 3과 연계).
- 명령(개념):
  ```c
  // syscall_count.c 에 추가
  BPF_ARRAY(want_syscall, u32, 1);   // 0xFFFFFFFF=전체, 그 외=그 번호만
  ```
  핸들러에서 `target` 검사 직후, `want_syscall[0]` 을 읽어 `*w != 0xFFFFFFFF && *w != args->id` 이면 `return 0`. 파이썬에서 `--syscall openat` 을 받아 번호로 변환해 주입(`syscall_name` 의 역방향 또는 BCC 의 번호 테이블 사용).
- 관찰: `--syscall openat` 으로 돌리면 `openat` 만 카운트되고 나머지는 사라지는지 본다.
- 질문: 이 필터를 **커널에서** 거는 것과 **사용자 공간 출력에서** 거르는 것(§3.2 의 `--comm` 처럼)의 차이는? 어느 쪽이 맵 항목 수와 오버헤드를 줄이는가?
