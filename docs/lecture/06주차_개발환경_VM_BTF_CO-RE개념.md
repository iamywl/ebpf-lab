...
> macOS 에서는 eBPF 를 돌릴 수 없다. 왜 리눅스 VM 이 필요한지, 그리고 "한 번 컴파일해서 어디서나 돈다(CO-RE)"는 마법이 어떻게 가능한지 그 밑바탕(BTF)을 이해한다.

last_updated: 2026-06-11

> 🧭 **이번 주 동선**  ·  📘 과목1 해당 없음  ·  📕 과목2 3주차 🔵 (11주차와 함께)
> - 🔬 **실습(VM `ssh ossca-ebpf`)**: [고급 01 libbpf execsnoop](../../labs/08_고급/README.md) (`make`)
> - 🧵 **OS 트랙 함께 보기**: —
> - ↔️ **이동**: ⬅️ [5주차 프로그램 타입](05주차_프로그램_타입과_부착지점.md) · 🏠 [강의 인덱스](README.md) · ➡️ [7주차 bpftrace 입문](07주차_bpftrace_입문.md)
...
> 이번 주는 7·8주차(bpftrace·BCC 입문)와 9·10주차(실습①·②)로 가기 전에 **"우리가 어디서, 무엇 위에서 실습하는가"** 를 단단히 다지는 주차다. 앞선 [4주차](04주차_eBPF_아키텍처_검증기_JIT_맵_헬퍼.md)에서 배운 검증기·맵·헬퍼, [5주차](05주차_프로그램_타입과_부착지점.md)에서 배운 프로그램 타입·부착지점이 *실제로 어느 커널 위에서 동작하는지* 손으로 확인해 본다.

---

## 1. 왜 리눅스 VM 인가 — macOS 에는 eBPF 가 없다

eBPF 는 **리눅스 커널 안에서 도는 작은 프로그램**이다. 즉 eBPF 라는 기능 자체가 리눅스 커널의 일부다. macOS 커널(XNU)에는 이 기능이 없다. 그래서 Mac 에서 `bpftrace` 같은 명령을 그냥 칠 수는 없고, **리눅스를 하나 띄워서 그 안에서** 실습해야 한다.

본 랩은 Apple Silicon Mac 에서 가볍게 리눅스를 띄우는 [`tart`](https://tart.run) 를 써서 Ubuntu 24.04 VM 을 운영한다. 전체 그림은 다음과 같다.
...
> 정리: **eBPF = 리눅스 커널 기술 → macOS 불가 → 리눅스 VM 필수.** 이게 이번 랩이 VM 으로 진행되는 이유 전부다.

---

## 2. eBPF 를 돌리려면 커널이 갖춰야 할 것

아무 리눅스나 다 되는 것은 아니다. 실습에 필요한 조건을 정리하면 다음과 같다.
...
VM 에 접속해 직접 확인하는 명령은 다음과 같다(이번 주 실습 과제에서 다시 다룬다).
...
> 💡 `/sys/kernel/btf/vmlinux` 파일 하나의 존재 여부가 "이 커널이 CO-RE 를 지원하느냐"의 핵심 신호다. 왜 그런지는 4·5절에서 설명한다.

---

## 3. tart 로 VM 띄우기 (본 랩 런북 연계)

자세한 사용법은 저장소 루트의 [README](../../README.md) 에 단계별로 정리돼 있다. 여기서는 핵심 흐름만 짚는다. 아래 `tart`/`ssh` 명령은 모두 **Mac 터미널**에서 친다.
...
접속한 뒤 VM 안에서 도구가 제대로 깔려 있는지 확인한다.
...
> ⚠️ macOS 에서는 위 도구가 동작하지 않는다. 반드시 `ssh ossca-ebpf` 로 **VM 안에 들어간 뒤** 실행한다.

---

## 4. 커널 헤더 방식의 문제 — 왜 이식성이 깨지나

eBPF 프로그램은 흔히 커널 내부 구조체(예: `struct task_struct`, `struct sock`)의 **특정 필드**를 읽는다. 그런데 이 구조체들의 **메모리 레이아웃(어떤 필드가 몇 바이트째에 있는가)** 은 커널 버전·빌드 설정에 따라 달라진다.

예를 들어 어떤 필드 `oom_score_adj` 의 위치(오프셋)가 커널 A 에서는 구조체 시작부터 24바이트째, 커널 B 에서는 32바이트째라고 하자. 전통적인 방식은 컴파일할 때 **그 커널의 헤더에서 오프셋을 박아 넣는다(하드코딩)**. 그러면 커널 A 용으로 컴파일한 eBPF 를 커널 B 에서 로드하면 엉뚱한 바이트를 읽게 된다.

> 🔬 **왜 오프셋이 커널마다 달라지나 — 구체적으로**: `struct task_struct` 같은 큰 구조체는 수백 개의 필드로 이뤄진다. 그 레이아웃은 다음 이유로 빌드마다 달라진다.
>
> - **앞쪽 필드가 추가/삭제됨**: 새 커널이 `oom_score_adj` *앞에* 새 필드를 하나 끼워 넣으면, 그 뒤의 모든 필드 오프셋이 밀려난다.
> - **`CONFIG_*` 빌드 옵션**: 어떤 필드는 특정 설정(`#ifdef CONFIG_...`)일 때만 구조체에 들어간다. 같은 커널 버전이라도 배포판이 켠 옵션이 다르면 레이아웃이 달라진다.
> - **타입 크기·정렬**: 컴파일러 정렬 규칙이나 필드 타입 변경으로 중간에 패딩이 달라질 수 있다.
>
> 즉 "커널 버전"만 같다고 레이아웃이 같다는 보장이 없다. **그 빌드의 실제 레이아웃**을 알아야만 올바른 오프셋을 쓸 수 있고, 그 정보를 담은 것이 바로 다음 절의 BTF 다.
...
이 문제 때문에 과거에는 **"실행할 그 머신에서, 그 커널의 헤더로, 매번 다시 컴파일"** 하는 방식을 썼다. BCC 가 바로 이 방식이다(런타임 컴파일). 정확하지만, 실행 머신마다 컴파일러(clang/LLVM)와 커널 헤더가 깔려 있어야 하고 로드할 때마다 컴파일 비용이 든다.

> 한 줄 요약: **구조체 레이아웃은 커널마다 다르다 → 한 번 컴파일한 바이너리가 다른 커널에서 깨진다.** 이것이 eBPF 이식성의 근본 난제다.

---

## 5. BTF — 커널이 "내 타입 정보"를 스스로 알려준다

**BTF(BPF Type Format)** 는 커널이 자기 안의 **타입 정보(구조체·필드·오프셋·크기 등)** 를 압축해 담아 둔 메타데이터 형식이다. `CONFIG_DEBUG_INFO_BTF=y` 로 빌드된 커널은 부팅 시 이 정보를 다음 파일로 노출한다.
...
`/sys/kernel/btf/vmlinux` 안에는 **"이 커널에서 `struct sock` 은 어떻게 생겼고, 어떤 필드가 몇 바이트째에 있는지"** 가 들어 있다. 즉, **지금 돌고 있는 바로 그 커널의 구조체 지도**다. 헤더 파일을 찾아 헤맬 필요 없이, 커널이 직접 알려주는 셈이다.
...
이 BTF 가 있기에 다음 절의 CO-RE 가 가능해진다. **"실행 시점에 이 커널의 실제 오프셋을 물어볼 수 있는 안내데스크"** 가 생긴 것이다.
...
4절의 그림과 비교해 보자. 4절에서는 오프셋을 컴파일 시점에 박아 넣어 다른 커널에서 깨졌다. CO-RE 는 오프셋 결정을 **로드 시점으로 미루고**, BTF 라는 안내데스크에 물어봐서 교정한다. 그래서 "Compile **Once**, Run **Everywhere**" 다.

> 비유: 이사 갈 때 "TV 는 거실 벽에서 24cm" 라고 못 박아 적어두면(헤더 방식) 새 집에서 틀어진다. 대신 "TV 는 *거실 콘센트 옆*" 처럼 **상대 위치만 적어두고**, 새 집에 도착해서 *그 집의 콘센트 위치(BTF)* 를 보고 맞추면(CO-RE) 어느 집이든 들어맞는다.

### 6.1 CO-RE 재배치의 종류 — 오프셋만이 아니다

CO-RE 가 로드 시점에 교정하는 것은 "필드 오프셋" 하나가 아니다. 커널 차이를 흡수하기 위한 **여러 종류의 재배치(relocation)** 가 있다.
...
핵심은 **필드 존재 여부** 재배치다. 이게 있어서, 신·구 커널을 모두 지원하는 프로그램을 "이 필드가 있는 커널에서는 이렇게, 없는 커널에서는 저렇게" 식으로 **하나의 바이너리 안에서 분기**할 수 있다. 단순 오프셋 교정을 넘어, *구조 자체의 차이*까지 흡수하는 것이 CO-RE 의 힘이다.

### 6.2 `vmlinux.h` — 커널 전체 타입을 하나의 헤더로

libbpf+CO-RE 로 코드를 짤 때는 커널 헤더 수십 개를 `#include` 하는 대신, **커널 BTF 에서 뽑아낸 단 하나의 헤더 `vmlinux.h`** 를 포함한다. 생성 방법은 간단하다.
...
이 한 줄이 의미하는 바가 크다. **커널 소스 헤더를 따로 설치할 필요가 없다** — 지금 돌고 있는 커널이 BTF 로 자기 타입을 다 알려주니, 그걸 그대로 헤더로 받아 적는 것이다. 그리고 `vmlinux.h` 에 적힌 오프셋은 컴파일 결과에 *박히지 않는다*. 컴파일러(clang)가 CO-RE 재배치 표시만 남기고, 실제 오프셋은 6절에서 본 대로 로드 시점에 채워지기 때문이다. 그래서 "개발 PC 의 `vmlinux.h` 로 빌드한 바이너리"가 "오프셋이 다른 운영 서버 커널"에서도 동작한다.

---

## 7. BCC vs libbpf+CO-RE — 두 가지 길 맛보기

본 랩의 실습①·②(9·10주차)는 **BCC** 로 작성돼 있고, 11주차에서 **libbpf+CO-RE** 를 다룬다. 두 방식을 비교해 둔다.
...
선택 기준을 한 줄로: **빠르게 배우고 탐색할 땐 BCC, 운영 환경에 작고 빠르게 배포할 땐 libbpf+CO-RE.** 둘 다 같은 BTF 인프라 위에서 돌지만, *언제 컴파일하느냐* 가 핵심 차이다.

> 🔬 **왜 BCC 는 "무겁다"고 하나**: BCC 는 실행 시점에 C 소스를 컴파일해야 하므로, 배포물 안에 **LLVM/clang 한 벌을 통째로** 끌고 다닌다(수백 MB 규모). 게다가 컴파일 순간에 그 머신의 **커널 헤더가 있어야** 구조체를 풀 수 있다 — 헤더가 없거나 버전이 어긋나면 그 자리에서 컴파일이 실패한다. 매 실행마다 컴파일하니 시작도 느리다. 반대로 **libbpf+CO-RE** 는 개발 PC 에서 **딱 한 번 미리 컴파일**해 작은 단일 바이너리를 만들고, 실행 머신엔 **BTF 만 있으면** 된다(헤더·컴파일러 불필요). 그래서 운영 서버 수천 대처럼 "커널 버전이 제각각이고 빌드 도구를 깔기 싫은" 환경에 적합하다.
>
> 정리하면, BTF 라는 같은 토대 위에서 ① BCC 는 *그 머신의 헤더로 매번 새로 컴파일해* 정확성을 얻고, ② libbpf+CO-RE 는 *재배치 표시를 남긴 뒤 로드 시점에 BTF 로 교정해* 이식성을 얻는다. "정확성을 그 자리에서 다시 빌드해 확보 vs 이식성을 재배치로 확보"가 두 길의 본질이다.

---

## ⚙️ 리눅스 커널은 자기 타입 정보를 BTF 로 품고 있다

eBPF 의 이식성은 커널 한복판에 들어 있는 작은 메타데이터 덩어리 하나에서 출발한다. 커널을 빌드할 때(`CONFIG_DEBUG_INFO_BTF=y`) **BTF(BPF Type Format)** 가 함께 생성되어 부팅 후 `/sys/kernel/btf/vmlinux` 로 노출된다. BTF 는 그 커널이 가진 **모든 구조체의 레이아웃 정보**(어떤 필드가 몇 바이트째에 있는가)를 담는다. 예를 들어 프로세스를 표현하는 `struct task_struct` 의 각 필드 위치가 여기에 적혀 있다. CO-RE 는 컴파일 시 오프셋을 박지 않고 재배치 표시만 남긴 뒤, 로드 시점에 이 BTF 를 읽어 **그 커널의 실제 오프셋으로 맞춰** 이식성을 얻는다.
...
소스/구조 측면에서, 이 파일은 커널 이미지(`vmlinux`)에 포함되어 빌드되며, 우리 VM(커널 6.17 aarch64)은 BTF 가 기본 활성화되어 `/sys/kernel/btf/vmlinux` 가 존재한다.

---

## 📸 실제 실행 화면 (실제 터미널 캡처)

아래는 VM(커널 6.17 aarch64)에 접속해 환경과 커널 구조체를 직접 확인한 모습이다.

![환경 한눈에 보기 — 실제 터미널 캡처](images/more/w6_env.png)

위는 `uname` / `bpftrace --version` / `/sys/kernel/btf/vmlinux` 존재를 한 화면에서 확인한 실제 터미널 캡처다. 커널 6.17 과 BTF 파일이 함께 보이면 CO-RE 실습 준비가 끝난 것이다.

![bpftool 로 본 struct task_struct — 실제 터미널 캡처](images/more/w6_btf_task.png)

위는 `bpftool btf dump` 로 커널 BTF 에서 `struct task_struct` 필드를 뽑아 본 실제 터미널 캡처다. 커널이 프로세스를 표현하는 실제 구조체와 그 필드들이 BTF 안에 그대로 들어 있음을 눈으로 확인할 수 있다.
...
> 모든 명령은 VM 안에서 실행한다. 먼저 Mac 터미널에서 `tart run ossca-ebpf-work --no-graphics &` 로 VM 을 켜고 `ssh ossca-ebpf` 로 접속한다.
...
- **명령**:
  ...
  ```
- **관찰**: 맨 처음엔 `struct task_struct;` (전방 선언)만 보일 수 있다. 필드가 가득한 **본 정의**는 덤프 뒤쪽에 나오므로, 정의 본문을 보려면 페이저로 넘겨 본다: `sudo bpftool btf dump file /sys/kernel/btf/vmlinux format c | sed -n '/^struct task_struct {/,/^};/p' | head -60`. 거기서 `pid`, `comm`, `oom_score_adj` 같은 필드가 줄줄이 나온다. 이 필드 순서·구성이 곧 "이 커널 빌드의 실제 레이아웃"이며, 4절에서 본 대로 **다른 커널이면 이 순서가 달라질 수 있다**. 같은 방식으로 `struct sock` 도 찾아본다.
...
**과제 4. (생각해 보기) — clang 이 없다면?** 과제 3 의 실습은 BCC 기반이다(런타임 컴파일). 만약 VM 에서 clang 을 지워버리면 어떤 일이 생길지 예상해 보고, libbpf+CO-RE 였다면 그 영향이 어떻게 달라질지 한 문단으로 적어라. *(주의: 실제로 clang 을 지우지는 말 것 — 실습 환경이 망가진다.)*
...
[7주차](07주차_bpftrace_입문.md)에서는 드디어 **bpftrace** 로 한 줄짜리 추적기를 직접 돌려본다. `tracepoint:raw_syscalls:sys_enter` 한 줄로 "어떤 프로세스가 시스템콜을 몇 번 부르나" 를 세어보는 등, awk 같은 고수준 언어로 커널을 들여다보는 첫 경험을 한다. 이번 주에 준비한 VM 과 BTF 가 그 무대다.
