...
> 🧭 **이번 주 동선**  ·  📘 과목1 해당 없음  ·  📕 과목2 10주차 🔵 (XDP 제작·강제)
> - 🔬 **실습(VM `ssh ossca-ebpf`)**: [고급 02 XDP 방화벽](../../labs/08_고급/README.md) (ICMP 드롭=강제)
> - 🧵 **OS 트랙 함께 보기**: —
> - ↔️ **이동**: ⬅️ [11주차 libbpf·CO-RE](11주차_libbpf와_CO-RE_프로덕션eBPF.md) · 🏠 [강의 인덱스](README.md) · ➡️ [13주차 보안 LSM](13주차_eBPF_보안_LSM_Falco_Tetragon.md)
...
### 1.1 더 깊이 — 패킷이 `sk_buff` 가 되기까지

각 단계를 조금 더 풀어 보자. 비용이 어디서 발생하는지가 곧 "왜 XDP 가 빠른가"의 답이다.

| 단계 | 무슨 일이 일어나나 | eBPF 훅 | 비용 |
|:---|:---|:---|:---|
| **NIC 하드웨어** | 랜선의 전기/광 신호를 프레임으로 수신, DMA 로 메모리(RX 링)에 적재 | (오프로드 XDP 일부) | 하드웨어 |
| **드라이버 RX** | RX 링에서 프레임을 꺼내 처리 시작 | **XDP**(여기!) | 매우 낮음 |
| **`sk_buff` 생성** | 패킷을 커널 표준 표현인 `struct sk_buff` 로 감쌈 — 헤더 포인터·메타데이터·소유권 정보가 붙는 **무거운 객체** | — | **여기서 급증** |
| **네트워크 스택** | L2/L3/L4 파싱, 라우팅, netfilter(iptables) 체인, 연결 추적 | **tc/clsact** | 중간~높음 |
| **소켓 큐** | 목적 소켓의 수신 버퍼에 적재, 앱이 `recv()` 로 읽음 | **socket/cgroup** | 앱 의존 |

- **핵심 분기점은 `sk_buff` 생성**이다. 이 객체를 만드는 순간 메모리 할당과 메타데이터 채우기로 비용이 확 뛴다. **XDP 는 바로 그 직전**, 드라이버 RX 단계에서 원시 프레임(`struct xdp_md` → `data`~`data_end` 바이트 영역)만 보고 판단한다. 그래서 버릴 패킷이라면 `sk_buff` 를 **아예 만들지 않고** 버려 비용을 0 에 가깝게 만든다.
- 반대로 iptables 룰은 스택 안(`sk_buff` 생성 후)에서 평가되므로, 같은 "드롭"이라도 이미 무거운 객체를 만든 뒤다. 이것이 §2 의 "XDP 가 iptables 보다 빠른 이유"의 본질이다.
...
### 2.1 XDP 동작 모드 — native / generic / offload

XDP 프로그램이 패킷 경로의 **어느 지점에서 실행되느냐**에 따라 세 가지 모드가 있다. 빠를수록 더 앞(NIC 에 가까움)에서 돈다.

| 모드 | 실행 위치 | 전제 조건 | 성능 | 비고 |
|:---|:---|:---|:---|:---|
| **native(드라이버)** | NIC 드라이버의 RX 경로, `sk_buff` 생성 **전** | 드라이버가 XDP 지원 | 매우 높음 | 프로덕션 표준. 대부분의 고속 사례가 이것 |
| **generic(일반/SKB)** | 네트워크 스택 진입부, **`sk_buff` 생성 후** 흉내 | 아무 드라이버나 가능 | 낮음 | 드라이버 미지원 시 폴백·개발/테스트용 |
| **offload(NIC)** | **NIC 하드웨어**가 직접 실행 | 스마트NIC(예: 일부 Netronome) | 최고 | CPU 를 거의 안 씀. 지원 NIC 가 드묾 |

- **native** 가 우리가 "XDP 가 빠르다"고 말할 때의 그 모드다 — `sk_buff` 가 만들어지기 전에 원시 프레임을 본다.
- **generic** 은 드라이버가 native 를 지원하지 않을 때 커널이 제공하는 **폴백**이다. 이미 `sk_buff` 가 만들어진 뒤라 XDP 본래의 속도 이점은 거의 사라지지만, **API/프로그램은 동일**해 개발·테스트에 유용하다. (학습용 VM 의 가상 NIC 도 보통 generic 으로 떨어진다.)
- **offload** 는 프로그램을 NIC 펌웨어에 내려 **호스트 CPU 를 아예 거치지 않고** 처리한다. 가장 빠르지만 지원 하드웨어가 희소하다.

> 정리: 성능은 **offload > native > generic**. 어느 모드로 붙었는지는 `bpftool net show` 나 `ip link` 출력에서 확인할 수 있다(예: `xdp`, `xdpgeneric`, `xdpoffload`).
...
네트워킹 eBPF 를 이해하려면 패킷이 커널 안에서 어떻게 올라오는지를 먼저 그려야 한다. 패킷은 **NIC → 드라이버 → 커널 네트워크 스택 → 소켓** 순으로 올라오며, 위로 갈수록 정보는 풍부해지지만 처리 비용도 커진다. 스택에 진입하면 커널은 패킷을 무거운 표현인 `sk_buff` 로 감싼다. eBPF 훅은 이 경로의 두 길목에 걸린다. **XDP** 는 `sk_buff` 가 만들어지기 전, 드라이버 최전선에 붙어 가장 빠르고, **tc(clsact)** 는 `sk_buff` 가 만들어진 뒤 큐잉 계층의 ingress/egress 양쪽에 붙어 풍부한 메타데이터를 다룬다.
...
소스/구조 측면에서, XDP 프로그램은 `struct xdp_md` 를, tc 프로그램은 `struct __sk_buff` 를 컨텍스트로 받는다. 우리 VM(커널 6.17 aarch64)에서 `bpftool net` 으로 부착 지점을 확인할 수 있다.

---

## 📸 실제 실행 화면 (실제 터미널 캡처)

아래는 VM(커널 6.17 aarch64)에서 네트워크 훅 지점과 TCP 상태 전이를 직접 들여다본 모습이다.

![bpftool net show — 실제 터미널 캡처](images/more/w12_net.png)

위는 `sudo bpftool net show` 로 XDP/tc 부착 지점을, `ip link` 로 인터페이스를 함께 확인한 실제 터미널 캡처다. 어떤 인터페이스의 어느 훅에 eBPF 가 붙어 있는지를 직접 볼 수 있다.

![tcpstates-bpfcc — 실제 터미널 캡처](images/more/w12_tcpstates.png)

위는 `tcpstates-bpfcc` 로 TCP 연결의 상태기계 전이(예: SYN_SENT → ESTABLISHED → CLOSE)를 추적한 실제 터미널 캡처다. 패킷이 스택을 오르내리며 소켓 상태가 어떻게 바뀌는지가 한 줄씩 기록된다.
...
> VM 접속: `ssh ossca-ebpf`(sudo 가능, Ubuntu 24.04 / 커널 6.17). 작업 경로: `~/ebpf-labs/{examples,labs,projects}`.

**① `bpftool net show` 로 XDP/tc 부착점 보기**

- 목표: 이 머신의 네트워크 인터페이스에 실제로 어떤 eBPF 훅(XDP·tc)이 붙어 있는지를 눈으로 확인한다.
- 명령:
  ```bash
  sudo bpftool net show          # XDP / tc(clsact) 부착 지점
  ip link                        # 인터페이스 목록 (xdp/xdpgeneric 표시 확인)
  ```
- 관찰: 출력의 `xdp`/`tc` 섹션에 부착된 프로그램 ID 가 보이는가? `ip link` 에서 어떤 인터페이스가 `xdpgeneric` 으로 표시되는가(§2.1 의 모드와 연결)?
- 질문: 아무 것도 안 붙어 있다면(빈 출력) 그 의미는? Cilium 같은 데이터패스가 깔린 노드라면 이 출력이 어떻게 달라질까?

**② `tcp_connect.bt` + `conn_summary.py` 로 나가는 연결 관측 (실습②의 축소판)**

- 목표: bpftrace 한 줄 버전과 파이썬 집계 버전으로 "누가 어디로 연결하나"를 두 각도에서 본다.
- 명령(창 1):
  ```bash
  cd ~/ebpf-labs
  sudo bpftrace examples/03_네트워크/tcp_connect.bt     # 나가는 TCP 목적지 IP:포트
  ```
  명령(창 2, 위가 도는 동안):
  ```bash
  sudo python3 labs/conn_summary.py     # 또 다른 창에서 curl 127.0.0.1:22 를 몇 번 반복
  ```
- 관찰: `tcp_connect.bt` 가 찍는 개별 목적지와, `conn_summary.py` 의 집계가 같은 트래픽을 일관되게 보여 주는가?
- 질문: 이 한 줄짜리 bpftrace 가 우리가 10주차에 BCC 로 만든 추적기와 본질적으로 같은 커널 지점(`tcp_v4_connect`)을 쓴다는 점을, Cilium/Hubble 의 "클러스터 흐름 관측"과 어떻게 연결할 수 있나?

**③ `tcpstates-bpfcc` 로 TCP 상태 전이 관찰**

- 목표: 패킷이 스택을 오르내릴 때 소켓 상태기계가 어떻게 바뀌는지(SYN_SENT → ESTABLISHED → CLOSE 등)를 한 줄씩 본다.
- 명령(창 1):
  ```bash
  ...
  명령(창 2):
  ```bash
  curl -s http://example.com > /dev/null || curl -s http://127.0.0.1:22 || true
  ```
- 관찰: 한 연결이 거치는 상태 전이 순서. 닫힌 포트로 시도하면 어떤 상태에서 멈추거나 곧장 `CLOSE` 로 가는가?
- 질문: 10주차 추적기는 `tcp_v4_connect` **진입**(연결 "시도")만 잡았다. `tcpstates` 가 보여 주는 상태 전이는 그보다 무엇을 더 알려 주는가(성공/실패, 수명)?

**④ 생각해 보기 — XDP 가 iptables 보다 빠른 이유**

- 같은 "특정 IP 드롭"을 ⓐ iptables 룰과 ⓑ `XDP_DROP` 으로 구현했다고 하자. §1.1 의 패킷 경로 표를 근거로, 두 방식이 패킷 경로의 **어느 지점**에서 드롭을 결정하는지, 그래서 `sk_buff` 생성 비용이 어느 쪽에서 발생/회피되는지 한 단락으로 설명하라. 룰이 수천 개로 늘어날 때 iptables(선형 체인)와 eBPF 맵 조회의 비용 증가가 어떻게 달라지는지도 덧붙여라.
