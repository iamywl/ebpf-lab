...
eBPF에서는 `perf_event` 타입 프로그램을 **타이머/PMU 이벤트**(PMU = Performance Monitoring Unit, CPU에 내장된 성능 측정용 하드웨어 카운터 — 사이클 수·캐시 미스 등을 센다)에 붙여 이 주기적 샘플링을 구현한다. 샘플이 발생할 때마다 eBPF 프로그램이 깨어나, 헬퍼 `bpf_get_stackid`(또는 `bpf_get_stack`)로 **현재 호출 스택**을 떠서 맵에 누적한다. 정확히는 스택 자체는 `BPF_MAP_TYPE_STACK_TRACE` 맵에 저장되고, 별도 카운트 맵이 그 **스택 ID별 빈도**를 센다(§4 히스토그램과 같은 맵 집계 구조다).
...
> 성능 분석의 핵심인 **히스토그램**과 **샘플링 프로파일**을 **실제 터미널에서 실행한 화면을 그대로 캡처**한 것이다.
