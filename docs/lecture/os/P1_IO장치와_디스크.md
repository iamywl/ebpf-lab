...
> 🧭 **이 모듈을 보는 시점**  ·  📘 과목1 10주차 🟢  ·  📕 과목2 해당 없음
> - 🔬 **실습(VM `ssh ossca-ebpf`)**: `sudo biolatency-bpfcc` · `sudo biosnoop-bpfcc`
> - ↔️ **OS 트랙 이동**: ⬅️ [C3 조건변수·세마포어](C3_조건변수_세마포어_이벤트기반.md) · 🏠 [OS 트랙 인덱스](README.md) · ➡️ [P2 파일시스템·VFS](P2_파일시스템과_VFS.md)
...
파일에 쓰거나 읽는 연산은 결국 **블록 I/O 요청**이 된다. 한 요청은 `struct bio`로 표현되는데, 이 구조체는 전송할 페이지들과 시작 섹터(`bi_iter.bi_sector`)를 담는다. 여러 `bio`는 `struct request`로 묶이고, 이 `request`는 디바이스마다 있는 `struct request_queue`에 들어간다. 최신 커널은 큐를 CPU별로 나눠 락 경합을 줄이는 멀티큐 블록 계층(`blk-mq`)을 쓴다.

요청이 흐르는 경로는 다음과 같다. 파일시스템 또는 페이지 캐시가 `submit_bio()`를 호출하면 요청이 I/O 스케줄러로 들어가 큐에서 정렬되고, 디바이스 드라이버가 이를 장치로 발행한다. 장치가 처리를 끝내면 인터럽트가 발생하고, 그 완료가 상위 계층으로 통지된다.

추적점 `block:block_rq_issue`(요청이 디바이스로 발행되는 순간)와 `block:block_rq_complete`(완료되는 순간)가 이 경로의 양 끝이다. 두 시점의 시간차가 `biolatency`가 재는 지연이며, 이는 다음 절의 🔬/💻 절에서 직접 측정한다.
