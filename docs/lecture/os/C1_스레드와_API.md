...
여기에 트레이드오프가 있다. 스레드는 생성·전환이 프로세스보다 싸고 메모리를 공유해 통신이 쉽다. 그런데 바로 그 공유가 동기화 부담과 데이터 레이스(두 스레드가 같은 메모리를 동시에 건드려 결과가 들쭉날쭉) 위험을 낳는다. 싸고 편한 만큼 조심할 거리가 생기는 것이다(→ C2 모듈에서 다룬다).
