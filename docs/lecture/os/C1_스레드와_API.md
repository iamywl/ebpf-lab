...
> OSTEP는 "스레드란 한 프로세스 안의 여러 실행 흐름"이라고 가르친다. 그런데 리눅스에서 그 스레드는 **실제로 무엇으로 만들어질까?** eBPF로 `clone()` 시스템콜을 잡아 직접 확인한다.

last_updated: 2026-06-12

> 🧭 **이 모듈을 보는 시점**  ·  📘 과목1 7주차 🟢  ·  📕 과목2 해당 없음
> - 🔬 **실습(VM `ssh ossca-ebpf`)**: `labs/04_동기화/` (clone() 추적)
> - ↔️ **OS 트랙 이동**: ⬅️ [V4 가상메모리](V4_가상메모리_주소공간_페이징.md) · 🏠 [OS 트랙 인덱스](README.md) · ➡️ [C2 락·동기화·버그](C2_락_동기화_그리고_버그.md)
...
OSTEP 26장은 **스레드**를 이렇게 소개한다. 하나의 프로세스 안에 PC(프로그램 카운터)와 레지스터, 그리고 **자기만의 스택**을 가진 실행 흐름이 여러 개 있는 것. 프로세스가 여러 개일 때와 비슷하지만, 결정적 차이는 **주소공간을 공유**한다는 점이다.
...
OSTEP는 이 개념을 코드 없이 먼저 **시뮬레이터**로 체감하게 한다.
...
OSTEP는 "스레드가 생긴다"고 말하지만, **리눅스 커널 입장에서 스레드 생성은 `clone()` 시스템콜 한 번**이다. 프로세스 생성(`fork`)과 같은 시스템콜을 쓰되, **무엇을 공유할지 플래그로 지정**하는 것이 차이다. glibc의 `pthread_create`는 내부에서 `CLONE_VM | CLONE_FS | CLONE_FILES | CLONE_SIGHAND | CLONE_THREAD ...` 플래그를 켠 `clone3()`을 호출한다. (`CLONE_VM`이 바로 "주소공간 공유"다.)

8개의 스레드를 만드는 프로그램을 돌리면서, `clone`/`clone3` 시스템콜이 몇 번 일어나는지 센다.
...
*위 이미지는 VM(커널 6.17)에서 직접 찍은 실제 터미널 캡처다.* 8개의 스레드를 만드는 프로그램에서 `@clone3[프로그램이름]`이 **약 8회** 잡힌다. (메인 스레드를 제외한 워커 스레드 수만큼이다.) 즉 OSTEP가 추상적으로 "스레드를 만든다"고 한 그 동작이, 커널에서는 **시스템콜 한 번 = 스레드 하나**로 1:1 대응한다.

> 배포판/glibc 버전에 따라 `clone`로 잡히기도, `clone3`로 잡히기도 한다. 두 트레이스포인트를 모두 거는 이유다. 둘 다 0이면 그 프로그램은 스레드를 안 만든 것이다.

### 🛠 직접 해보기

1. 위 bpftrace를 켠 채로, 스레드 개수를 4 → 8 → 16으로 바꿔 가며 프로그램을 실행해 본다. `@clone3` 값이 스레드 수에 따라 변하는가?
2. `fork()`를 쓰는 프로그램(예: 셸에서 `ls`)을 실행하면 같은 트레이스포인트에 잡힐까? `sys_enter_clone`에 `comm`을 추가해 직접 확인한다. (힌트: `fork`도 내부적으로 `clone`을 쓴다.)
...
27장은 **POSIX 스레드(pthread) API**를 다룬다. 핵심은 두 함수다.
...
`threads-api` 숙제 디렉터리에는 시뮬레이터가 아니라 **실제 C 코드**가 들어 있다(`main-race.c` 등). 직접 컴파일해 돌린다.
...
`pthread_create`는 **라이브러리 함수**이지 시스템콜이 아니다. 그래서 시스템콜 추적만으로는 "라이브러리 호출 → 커널 진입"의 경계가 안 보인다. 두 층위를 나란히 보면 이렇게 대응한다.
...
- **사용자공간 층** `pthread_create`: glibc의 uprobe로 잡을 수 있다.
...
이상적으로는 **uprobe로 잡은 `pthread_create` 횟수 ≈ tracepoint로 잡은 `clone3` 횟수**가 된다. "라이브러리 호출 1번이 시스템콜 1번으로 내려간다"는 것을, 두 탐침을 동시에 걸어 눈으로 확인할 수 있다.

> 우리 데모 프로그램 `threads_lock.c`는 OSTEP `threads-api`의 `main-race.c`를 **여러 스레드 + 반복 루프**로 확장한 것이다. C2 모듈에서 락 경합을 만들 때 다시 쓴다.

### 🛠 직접 해보기

1. uprobe(`pthread_create`)와 tracepoint(`clone3`)를 **한 bpftrace 명령**에 함께 걸고, 8스레드 프로그램에서 두 카운트가 같은지 비교한다.
2. `pthread_join`을 일부러 빼면 어떤 일이 생기는가? (워커가 다 끝나기 전에 메인이 종료 → 결과 유실) 직접 코드를 고쳐 출력 차이를 관찰한다.
...
[C2 · 락·동기화 그리고 버그](C2_락_동기화_그리고_버그.md) — 공유 변수를 여러 스레드가 건드릴 때 생기는 **데이터 레이스**를 눈으로 보고, 락 경합이 커널 `futex` 시스템콜로 드러나는 과정을 추적한다.
