...
> 🧭 **이 모듈을 보는 시점**  ·  📘 과목1 12주차 🟢  ·  📕 과목2 9주차 🔵 (ext4 jbd2 저널)
> - 🔬 **실습(VM `ssh ossca-ebpf`)**: `labs/05_파일IO/fsync_trace.py` · `sudo ext4slower-bpfcc`
> - ↔️ **OS 트랙 이동**: ⬅️ [P2 파일시스템·VFS](P2_파일시스템과_VFS.md) · 🏠 [OS 트랙 인덱스](README.md) · ➡️ (트랙 끝 → 정규 강의로)
...
ext4의 저널링은 별도 계층인 **jbd2**가 담당한다. jbd2는 변경을 모아 `transaction_t`(트랜잭션)로 만들고, 커밋 시 저널 영역에 먼저 쓴 뒤(write-ahead) 체크포인트 단계에서 본래 위치에 반영한다. 크래시 후 마운트할 때는 저널을 재생(replay)해 일관성을 복구한다.

저널 모드는 세 가지이고 안전성과 성능 사이에서 트레이드오프가 다르다. `journal` 모드는 데이터까지 저널에 적어 가장 안전하지만 모든 데이터를 두 번 쓴다. `ordered` 모드는 기본값으로, 메타데이터만 저널링하되 데이터를 메타데이터 커밋 전에 디스크에 먼저 내린다. `writeback` 모드는 메타데이터만 저널링하고 순서를 보장하지 않아 빠르지만 약하다.

페이지 캐시 쪽을 보면, 파일 데이터는 `inode.i_mapping`(`struct address_space`)에 페이지 단위로 캐시된다. 쓰기는 일단 여기에 dirty 상태로 모였다가 백그라운드 writeback이나 `fsync`로 디스크에 내려간다.

`fsync()`의 커널 경로는 `vfs_fsync()` → `file->f_op->fsync()`로 이어지고, ext4가 해당 데이터와 저널을 디스크에 강제로 flush한다. 이 강제 flush 비용을 `ext4slower`가 잡는다. 그 측정은 아래 🔬 절에서 본다.
...
2절에서 `ext4slower`의 `S` 연산으로 fsync를 봤고, 4절에서 페이지 캐시의 write-back 때문에 `fsync`가 필요하다는 것을 봤다. 그 `fsync`가 바로 **"캐시에만 있던 데이터를 디스크에 진짜로 박아넣어라"** 라는 명령이고, 저널 commit과 맞물려 비용을 낸다. `labs/05_파일IO/fsync_trace.py`는 그 fsync의 **횟수와 소요시간**을 프로세스별로 집계한다 — durability의 가격표를 직접 재는 도구다.

### ① 커널에서 도는 부분 (eBPF C)

`fsync` 시스템콜의 진입~반환 시간을 재서, 프로세스별로 횟수·총시간·최대시간을 누적한다. C2의 futex 측정과 같은 "진입 시각 기억 → 반환에서 차이" 패턴이다.
...
- **부착** `sys_enter_fsync` / `sys_exit_fsync`: 응용이 "지금까지 쓴 걸 디스크에 확실히 박아라"고 호출하는 `fsync`의 들어가고 나오는 순간. 이 구간이 곧 **데이터를 페이지 캐시에서 디스크로 내리고 저널 commit이 끝나기를 기다리는 시간**이다.
- **진입 시각 기억** `start.update(&id, &ts)`: 진입 때 `bpf_ktime_get_ns()` 시각을 `pid_tgid` 키로 저장.
- **집계** `d = bpf_ktime_get_ns() - *tsp`: 반환−진입 = 이번 fsync가 걸린 시간. `total_ns`에 합산하고, `if (d > s->max_ns) s->max_ns = d`로 **최악 지연**도 따로 기록한다. 평균만 보면 가려지는 "가끔 튀는 긴 fsync"를 잡기 위해서다.
- **정리** `start.delete(&id)`: 짝지은 진입 기록 삭제.

fsync가 "디스크에 박아넣기"인 만큼, 이 `total_ns`/`max_ns`가 곧 durability를 보장하느라 응용이 멈춰 있던 비용이다.

### ② 사용자 공간 부분 (Python)

기다린 뒤 `stats` 맵을 읽어 총시간 큰 순으로, 횟수·총시간(ms)·최대(ms)를 출력한다.
...
- `time.sleep(args.duration)`: 그동안 커널이 `stats`를 누적(집계 방식).
- `v.total_ns/1e6`, `v.max_ns/1e6`: 나노초를 밀리초로 변환.
- `rows.sort(key=lambda r: -r[3])`: 총 fsync 시간이 큰 프로세스가 위로 → **디스크 동기화에 시간을 가장 많이 쓴 DB·로그·에디터**가 한눈에. fsync가 한 건도 없으면 안내 문구를 띄운다.

### 직접 실행

```bash
sudo python3 labs/05_파일IO/fsync_trace.py --duration 8
# 테스트: 다른 창에서  for i in 1 2 3; do dd if=/dev/zero of=/tmp/x bs=1M count=16; sync; done
```

기대 결과: `dd`+`sync`를 돌린 프로세스(또는 sync)가 fsync `횟수`·`총시간(ms)`·`최대(ms)`와 함께 찍힌다 — 4절에서 본 "write-back된 캐시를 디스크에 박아넣는 비용"이 프로세스별 숫자로 드러난다.
