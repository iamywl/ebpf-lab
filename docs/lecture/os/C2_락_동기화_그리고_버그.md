...
사용자 공간 뮤텍스는 경합이 없으면 원자연산(CAS)만으로 끝나고 커널에 들어가지 않는다. 경합이 생길 때만 `futex()` 시스템콜로 커널에 들어간다. 그래서 futex 호출량은 경합의 온도계가 된다.

커널 경로는 다음과 같다. `futex()` 시스템콜은 `do_futex()`(`kernel/futex/`)를 거쳐 연산에 따라 `futex_wait()` 또는 `futex_wake()`로 갈라진다.

```c
/* kernel/futex/ — 대기자는 futex_q 로 표현되어 해시 버킷 큐에 매달린다 */
struct futex_q {
    struct plist_node    list;        /* futex_hash_bucket 큐의 노드 */
    struct task_struct  *task;        /* 잠든 대기자 */
    union futex_key      key;         /* uaddr 를 해시한 키 */
    /* ... */
};
/* futex_hash_bucket : 잠금 주소(uaddr)를 해시해 같은 버킷에 모은다 */
```

대기하는 스레드는 `struct futex_q`로 표현되어, 잠금 주소(uaddr)를 해시한 `struct futex_hash_bucket`의 큐에 매달려 `TASK_INTERRUPTIBLE` 상태로 잠든다. 깨우는 쪽은 같은 버킷에서 대기자를 찾아 깨운다.

`pthread_mutex`·`pthread_cond`·POSIX 세마포어는 모두 이 futex 위에 구현된다. 추상 이름은 달라도 커널이 보는 원시연산은 futex 하나다(이 점은 3절에서 다시 본다).

🔬 절에서는 `sys_enter_futex`/`sys_exit_futex`를 재서 경합으로 잠든 시간(off-CPU 대기)을 측정한다(`futex_contention`/`offcputime`).
