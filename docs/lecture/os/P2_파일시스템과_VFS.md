...
> 🧭 **이 모듈을 보는 시점**  ·  📘 과목1 9주차 🟢  ·  📕 과목2 8주차 🔵 (inode·VFS 심화)
> - 🔬 **실습(VM `ssh ossca-ebpf`)**: `labs/05_파일IO/vfs_rw.py` · `sudo statsnoop-bpfcc` · `sudo vfsstat-bpfcc`
> - ↔️ **OS 트랙 이동**: ⬅️ [P1 디스크 I/O](P1_IO장치와_디스크.md) · 🏠 [OS 트랙 인덱스](README.md) · ➡️ [P3 저널링·캐시](P3_크래시일관성_저널링_캐시.md)
...
39장은 파일시스템의 **인터페이스**를 가르친다. 파일은 "이름이 붙은 바이트 배열"이고, 커널 내부에서는 **inode**라는 자료구조가 그 파일의 메타데이터(크기·소유자·권한·데이터 블록 포인터)를 담는다. 이름→inode 매핑은 디렉터리가 한다. 디렉터리도 사실은 (이름, inode 번호) 엔트리들을 담은 특수한 파일이다.

inode가 데이터 블록을 가리키는 방식도 짚어 둔다. inode 안에는 직접 블록 포인터가 몇 개 들어 있고(작은 파일은 이걸로 충분하다), 그것으로 모자라는 큰 파일은 간접·이중간접 포인터로 가리킨다(multi-level index). 직접 포인터는 데이터 블록을 곧장 가리키고, 간접 포인터는 "데이터 블록 포인터들을 담은 블록"을 가리켜 한 단계 더 들어간다. 그래서 inode 하나 크기는 작게 유지하면서도 아주 큰 파일까지 표현할 수 있다.
...
40장은 파일시스템을 **실제로 구현**한다. 디스크를 블록 배열로 보고, **슈퍼블록(superblock — 파일시스템 전체 정보(블록 크기·총 블록 수·inode 개수)를 담은 머리말 블록) / inode 비트맵 / 데이터 비트맵 / inode 테이블 / 데이터 블록** 영역으로 나눈다. 이 단순 파일시스템이 vsfs(Very Simple File System)다. 파일을 읽으려면 inode를 찾고 → 블록 포인터를 따라가 → 데이터 블록을 읽는 일련의 과정을 손으로 따라간다.
...
이 주제가 왜 나왔는지부터 보자. 파일을 디스크 어디에 어떻게 배치하느냐가 성능을 좌우한다. 옛 파일시스템(vsfs/old UFS)은 inode와 그 데이터 블록을 디스크상 멀리 떨어뜨려 둬서, 한 파일을 읽어도 디스크 헤드가 멀리 왕복(탐색 폭증)해 느렸다. 이를 개선한 것이 FFS(Fast File System)다. 실린더 그룹으로 관련 inode와 데이터 블록을 가까이 모아 지역성을 높였다. 트레이드오프는 지역성을 위해 공간 일부를 예약하고, 아주 큰 파일은 한 그룹에 다 못 담아 여러 그룹에 나눠 담는다는 점이다.
