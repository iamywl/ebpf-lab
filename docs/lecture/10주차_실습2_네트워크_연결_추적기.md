...
> 🧭 **이번 주 동선**  ·  📘 과목1 11주차 🟢 (실습②)  ·  📕 과목2 해당 없음
> - 🔬 **실습(VM `ssh ossca-ebpf`)**: `projects/netflow-tracer` (`sudo python3 verify_net.py`) · `labs/06_네트워크/conn_summary.py`
> - 🧵 **OS 트랙 함께 보기**: — (네트워크 주차는 병행 OS 모듈 없음)
> - ↔️ **이동**: ⬅️ [9주차 실습①](09주차_실습1_시스템콜_추적기.md) · 🏠 [강의 인덱스](README.md) · ➡️ [11주차 libbpf·CO-RE](11주차_libbpf와_CO-RE_프로덕션eBPF.md)

> 🔰 **1학년·입문자: 두 갈래 길로 읽자.**
> - **🟢 돌려보기 트랙(누구나)**: [§5 직접 실행해 보기](#5-직접-실행해-보기)를 복붙해 "누가 어디로 접속하는지"가 실시간으로 찍히는 걸 직접 보자. TCP·소켓·IP/포트 용어는 [용어 사전](00c_용어집_약어사전.md) 참고.
> - **🔵 이해 트랙(심화)**: §2~3 코드 분석. 특히 **바이트 오더(IP 가 왜 거꾸로 보이나)** 가 이번 주 최대 난관이니 [C 미니부록](00b_준비_C언어_미니부록.md)과 본문 §3.1 설명을 천천히 보자. C 가 낯설면 미니부록 먼저.
...
> 🔎 **심화 — 왜 검증기는 직접 역참조를 금지하나.** eBPF 프로그램은 **커널 주소 공간** 안에서 돈다. 만약 `sin->sin_family` 처럼 포인터를 직접 따라가게 허용하면, 그 포인터가 잘못된 값(null, 해제된 메모리, 사용자 공간 주소)일 때 **커널 전체가 패닉**할 수 있다. 그래서 검증기는 "신뢰할 수 없는 포인터"의 직접 역참조를 **로드 시점에 거부**한다. 대신 헬퍼 `bpf_probe_read_kernel(dst, size, src)` 는 내부적으로 **페이지 폴트를 안전하게 처리(fault-safe)** 한다 — 읽기가 실패하면 0 이 아닌 에러 코드를 돌려줄 뿐, 커널은 멀쩡하다. 정리하면 두 단계에서 막힌다. **① 로드 시점**: 검증기가 직접 역참조 자체를 거부 → 그런 프로그램은 커널에 올라가지도 못한다. **② 런타임**: 헬퍼가 잘못된 주소를 읽어도 보호 장치가 작동해 죽지 않는다.
>
> 참고: 커널 5.5 이전엔 `bpf_probe_read` 하나뿐이었지만, 지금은 출처를 명시하는 `bpf_probe_read_kernel`(커널 메모리)·`bpf_probe_read_user`(사용자 메모리)로 갈라졌다. `uaddr` 는 `connect(2)` 가 커널 안으로 복사해 둔 **커널 측** 사본이므로 `_kernel` 변형을 쓴다.
...
> 🔎 **심화 — 빅엔디언과 "127.0.0.1 이 왜 뒤집혀 보이나".** **네트워크 바이트 오더 = 빅엔디언**(big-endian: 큰 자리 바이트가 앞에)이다. 인터넷 표준이 이것으로 못박혀 있어, 리틀엔디언 CPU(x86·arm)든 빅엔디언 CPU 든 선 위에서는 항상 같은 순서로 IP·포트를 주고받는다. 문제는 우리 호스트(리틀엔디언)가 그 바이트들을 **정수로 해석할 때** 순서를 뒤집어 읽는다는 점이다.
>
> `127.0.0.1` 을 예로 들면, 네트워크 오더의 4바이트는 `7F 00 00 01`(= 127, 0, 0, 1) 이다. 리틀엔디언 호스트가 이걸 32비트 정수로 그냥 읽으면 **`0x0100007F`**(가장 낮은 주소의 바이트를 가장 낮은 자리로 본다)가 되어, 정수를 다시 점-십진으로 순진하게 풀면 `1.0.0.127` 로 뒤집혀 보인다. 그래서:
>
> - `struct.pack("<I", daddr)` — 호스트 정수 `0x0100007F` 를 **리틀엔디언 바이트 배열** `7F 00 00 01` 로 도로 펼친다(= 원래 네트워크 오더 복원).
> - `socket.inet_ntoa(...)` — 그 4바이트를 앞에서부터 `127.0.0.1` 로 읽는다.
>
> 포트도 같은 이치다. `ntohs`(network-to-host short)는 16비트 두 바이트의 자리를 뒤집는다 — 22(`0x0016`)를 네트워크 오더로 받으면 호스트에선 `0x1600`=5632 로 보이므로, `ntohs` 로 되돌려야 22 가 된다. C 쪽 workload 가 `htons(port)`·`inet_pton` 으로 **호스트→네트워크** 변환을 하는 것과 정확히 대칭이다(`h`=host, `n`=network).
...
> 🔎 **심화 — perf 이벤트 손실(lost)의 메커니즘.** perf 버퍼는 **CPU 마다** 별도의 고정 크기 링버퍼다. 커널이 `perf_submit` 으로 이벤트를 넣는 속도가, 사용자 공간이 `perf_buffer_poll` 로 빼내는 속도보다 빠르면 버퍼가 가득 찬다. 이때 커널은 새 이벤트를 **덮어쓰지 않고 버리며**, 버린 개수를 따로 세어 둔다. 그 수가 다음 poll 때 `lost_cb(n)` 로 전달된다. 즉 `lost` 는 "추적기가 느려서 놓친 양"의 정직한 지표다. 대응책은 ① **버퍼 크기 키우기**(`open_perf_buffer(..., page_cnt=...)`, 2의 거듭제곱 페이지 수), ② **이벤트 줄이기**(커널에서 미리 필터링해 꼭 필요한 것만 submit), ③ poll 루프를 가볍게 유지(콜백에서 무거운 작업 금지)다. 11주차의 **ring buffer** 는 이 문제를 구조적으로 완화한다(공유 버퍼·예약/제출).

> 🔎 **심화 — `tcp_v4_connect` 진입 = "연결 시도", 성공과 무관.** 우리는 함수 **진입점(kprobe)** 에 후킹하므로, 3-way 핸드셰이크가 성공했는지(`SYN→SYN/ACK→ACK`)와 **상관없이** 잡는다. 닫힌 포트라 `RST` 를 맞아 실패해도, 방화벽에 막혀 타임아웃이 나도, **시도한 순간** 한 건이 찍힌다. 보안 관점에서 이게 핵심이다 — 포트 스캔·정찰처럼 "실패하는 연결"이야말로 탐지하고 싶은 신호다. 반대로 "성공한 연결만" 보고 싶다면 **반환점(kretprobe)** 에서 반환값이 0 인지까지 확인해야 한다(확장 과제 4).
...
> 아래는 이 추적기를 **실제 터미널에서 실행한 화면을 그대로 캡처**한 것이다. `curl` 등 서로 다른 프로세스의 연결이 PID·목적지별로 잡힌다.
...
> VM 접속: `ssh ossca-ebpf`(sudo 가능). 작업 경로: `~/ebpf-labs/{examples,labs,projects}`.

**① 자기검증 200회 — 거짓말 불가 기준값 확인**

- 목표: 추적기가 시도한 연결을 빠짐없이 잡는지(관측 ≥ 시도) 음성 통제로 검증한다.
- 명령:
  ```bash
  cd ~/ebpf-labs/projects/netflow-tracer
  sudo python3 verify_net.py 200
  ```
- 관찰: 출력의 "workload 연결 시도", "추적기 관측", "추적기가 본 전체 이벤트 수", PASS/FAIL.
- 질문: "관측 = 시도" 와 "전체 이벤트 수 = 관측"이 각각 무엇을 보장하는가? 둘 중 하나만 어긋나면 어떤 의미인가?

**② 실시간 추적 켜고 다른 창에서 트래픽 발생**

- 목표: 서로 다른 프로세스의 연결이 PID·이름·목적지별로 구분돼 찍히는지 본다.
- 명령(창 1):
  ```bash
  sudo python3 netflow.py --duration 10
  ```
  명령(창 2, 위가 도는 동안):
  ```bash
  curl -s http://127.0.0.1:22 || true       # 열린/닫힌 포트 시도
  curl -s http://127.0.0.1:65000 || true    # 닫힌 포트(실패) 시도
  ```
- 관찰: `curl` 프로세스의 연결이 `net_workload` 와 별개로 잡히는가? 닫힌 포트(65000)로의 **실패한** 시도도 찍히는가?
- 질문: 실패한 연결까지 잡히는 이유는 무엇이며(§3.2 심화), 이것이 보안 탐지에 왜 유용한가?

**③ labs/conn_summary.py 로 집계 교차 비교**

- 목표: 같은 현상을 다른 도구로 집계해 우리 추적기 결과와 대조한다.
- 명령:
  ```bash
  cd ~/ebpf-labs
  sudo python3 labs/conn_summary.py        # 다른 창에서 curl 을 몇 번 반복
  ```
- 관찰: `conn_summary.py` 가 본 목적지별 연결 수와, `netflow.py` 의 PID별 합계가 같은 트래픽에 대해 정합적인가?
- 질문: 두 도구가 같은 커널 이벤트를 보면서도 집계 축(PID별 vs 목적지별)이 다르면, 어떤 분석에 어느 쪽이 유리한가?

**④ 생각해 보기 — IPv6 확장**

- 우리 추적기는 `tcp_v4_connect` 만 잡는다. `::1`(IPv6 루프백)로의 `curl -g 'http://[::1]:22'` 는 왜 안 잡히나? `tcp_v6_connect` 핸들러를 추가하려면 `sockaddr_in6` 의 어떤 필드(16바이트 `sin6_addr`)를 어떻게 읽고, `event_t` 와 사용자 공간 출력(`inet_ntop(AF_INET6, ...)`)을 어떻게 바꿔야 하는지 설계만 적어 보라.
